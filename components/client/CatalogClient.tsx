...
export default function CatalogClient({
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {/* ── HERO ── */}
      <section className="bg-gradient-to-br from-[#2d1f2e] via-[#6b3050] to-[#a06070] text-white">
        <div className="max-w-6xl mx-auto px-4 py-10 sm:py-16 md:py-20 flex flex-col md:flex-row items-center gap-8 md:gap-12">
          ...
          >
            ...
            <p className="text-rose-200 text-base sm:text-lg mb-8 max-w-md mx-auto md:mx-0">{t.sub_tagline}</p>
            <a href="#catalogue">
              <Button size="lg" className="bg-white text-brand-accent hover:bg-brand-secondary font-bold gap-2">
        ...
        <div className="max-w-6xl mx-auto px-4 py-5 grid grid-cols-1 sm:grid-cols-3 gap-5">
          {TRUST_BADGES.map(({ icon: Icon, label, sub }) => (
            <div key={label} className="flex items-center gap-3">
              <div className="w-10 h-10 bg-brand-primary/30 rounded-full flex items-center justify-center shrink-0">
      ...
      <Link href={`/products/${product.slug}`} className="group block h-full">
        <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all border border-gray-100 h-full flex flex-col">
          ...
          <div className="p-4 sm:p-5 flex flex-col flex-1">
            ...
            <h3 className="font-bold text-gray-900 text-sm sm:text-base leading-snug mb-1 line-clamp-2 group-hover:text-brand-accent transition-colors flex-1">{product.name}</h3>
            ...
            <Button className="w-full bg-brand-accent hover:bg-[#2a2a2a] text-white gap-2 text-sm" disabled={product.stock === 0} tabIndex={-1}>
